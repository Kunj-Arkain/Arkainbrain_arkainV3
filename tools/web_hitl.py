...
def _get_db():
    """Get a database connection and ensure the reviews table exists.
    
    This runs CREATE TABLE IF NOT EXISTS on every call — it's essentially
    free (SQLite just checks metadata) but guarantees the table is always
    present regardless of init order, deployment timing, or DB resets.
    """
    _REVIEWS_DDL = """
    ...
    try:
        ...
        # Ensure reviews table exists via the wrapper
        try:
            db.executescript(_REVIEWS_DDL)
            db.commit()
        except Exception:
            pass  # Table already exists or DB is read-only — either way, proceed
        return db
    except ImportError:
...
def init_reviews_table():
    ...
    Delegates to _get_db() which handles this automatically now."""
    try:
        db = _get_db()
        db.close()
    except Exception as e:
        logger.warning(f"init_reviews_table: {e}")
...
def get_pending_reviews(job_id: str = None) -> list[dict]:
    ...
    Returns empty list if reviews table doesn't exist (defensive)."""
    try:
        db = _get_db()
        if job_id:
            rows = db.execute(
        ...
        logger.warning(f"get_pending_reviews failed (returning empty): {e}")
        return []


def get_review(review_id: str) -> dict:
    """Get a single review by ID. Returns None if not found or table missing."""
    try:
        db = _get_db()
        row = db.execute(
            "SELECT r.*, j.title as job_title, j.output_dir FROM reviews r "
        ...
        logger.warning(f"get_review failed: {e}")
        return None
